...
import { notFoundSchema, ZOD_ERROR_CODES, ZOD_ERROR_MESSAGES } from "../../lib/constants.js";
export const list = async (c) => {
    ...
    if (search) {
        ...
        whereConditions.push(sql `(${tasks.name} LIKE ${searchPattern} OR ${tasks.description} LIKE ${searchPattern})`);
    }
...
export const listChildren = async (c) => {
    ...
    const [tasksList, totalResult] = await Promise.all([
        ...
        db.select({ count: sql `count(*)` }).from(tasks).where(eq(tasks.parentId, id)).get(),
    ]);
    if (!totalResult) {
        return c.json({
            data: [],
...
export const getOne = async (c) => {
    ...
    if (!result.success) {
        return c.json(createErrorSchema(IdUUIDParamsSchema).parse({
            success: false,
            error: result.error,
    ...
    if (!task) {
        return c.json(notFoundSchema.parse({
